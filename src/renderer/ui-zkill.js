...
async function populateCharFilter() {
  ...
  try { 
    chars = await window.electronAPI.getCharacters();
    if (typeof window.appendDebug === 'function') {
      window.appendDebug('INFO', 'Zkill: Fetched ' + chars.length + ' characters from registry');
      var onlineCount = chars.filter(function(c) { return c.online; }).length;
      window.appendDebug('INFO', 'Zkill: ' + onlineCount + ' characters online');
    }
  } catch (err) { 
    if (typeof window.appendDebug === 'function') {
      window.appendDebug('WARN', 'Zkill: Failed to fetch characters: ' + err.message);
    }
    return; 
  }
  ...
  // Restore selection if the character is still online, otherwise fallback to 'All online'
  charFilter.value = cur;
  
  if (typeof window.appendDebug === 'function') {
    window.appendDebug('INFO', 'Zkill: Populated dropdown with ' + onlineChars.length + ' online characters');
  }
}
...
window.loadZkillSettings = function (settings) {
  ...
  // Refresh character list every 30 seconds to catch presence updates
  if (!charRefreshTimer) {
