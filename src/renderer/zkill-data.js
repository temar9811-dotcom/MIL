...
function log(level, msg) {
    if (typeof window.appendDebug === 'function') window.appendDebug(level, 'Zkill-Data: ' + msg);
}
...
function systemsWithin(systemNames, maxJumps) {
    ...
    log('DEBUG', 'Calculating systems within ' + maxJumps + ' jumps of ' + systemNames.length + ' origin systems.');
    ...
    log('DEBUG', 'Found ' + out.size + ' total systems in range.');
    return out;
}

async function init() {
    try {
        if (!window.electronAPI || !window.electronAPI.getSystemsAdjacency) {
            log('WARN', 'electronAPI.getSystemsAdjacency not available.');
            return;
        }
        log('INFO', 'Loading systems adjacency map from main process...');
        var raw = await window.electronAPI.getSystemsAdjacency();
        buildAdjacency(raw);
        log('INFO', 'Adjacency map loaded successfully (' + adjacency.size + ' systems).');
    } catch (err) {
        log('ERROR', 'Adjacency load failed: ' + err.message);
    }
}
